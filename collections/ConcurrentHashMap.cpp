...
static size_t _get_data_count(ConcurrentHashMap& map)
{
...
static void _init_group(ConcurrentHashMap& map, size_t group_index)
{
	...
	static uint8_t empty[32] = {
		(uint8_t)kEmpty, (uint8_t)kEmpty, (uint8_t)kEmpty, (uint8_t)kEmpty,
		(uint8_t)kEmpty, (uint8_t)kEmpty, (uint8_t)kEmpty, (uint8_t)kEmpty,
		(uint8_t)kEmpty, (uint8_t)kEmpty, (uint8_t)kEmpty, (uint8_t)kEmpty,
		(uint8_t)kEmpty, (uint8_t)kEmpty, (uint8_t)kEmpty, (uint8_t)kEmpty,
		(uint8_t)kEmpty, (uint8_t)kEmpty, (uint8_t)kEmpty, (uint8_t)kEmpty,
...
// Creates the Vec at 'index' and initialises a per-slot Mutex for every slot.
// Vec element size is map.entry_size so each slot is [Mutex | key | value].
static int _init_vec(ConcurrentHashMap& map, size_t index)
{
	Vec* vec = map.data + index;

	int error = create_vec(*vec, map.entry_size, map.data_vec_size);
	if (error != E_OK)
		return error;

	vec->size = map.data_vec_size;

	for (size_t i = 0; i < map.data_vec_size; i++)
	{
		void* slot = (uint8_t*)vec->data + i * map.entry_size;
		create_mutex(*(Mutex*)slot);
	}

	return E_OK;
}

// Destroys per-slot mutexes then the Vec itself.
static void _destroy_vec(ConcurrentHashMap& map, size_t index)
{
	Vec* vec = map.data + index;
	...
	{
		void* slot = (uint8_t*)vec->data + i * map.entry_size;
		destroy_mutex(*(Mutex*)slot);
	}

	destroy_vec(*vec);
}

static int _hashmap_grow(ConcurrentHashMap& map)
{
...
static int _hashmap_add_free_indexes(ConcurrentHashMap& map, size_t from_capacity)
{
...
// Runs under already-held global_mutex — no extra locking required inside.
static int _hashmap_rehash(ConcurrentHashMap& map)
{
	...
	{
		j = *(size_t*)vec_get(map.occupied_indexes, i, error);

		size_t data_index = j / map.data_vec_size;
		size_t elm_index = j - data_index * map.data_vec_size;

		Vec* vec = map.data + data_index;
		// entry start: [Mutex | key | value]; key begins at sizeof(Mutex)
		void* element = (uint8_t*)vec->data + elm_index * map.entry_size;

		size_t hash = map.hfunc((uint8_t*)element + sizeof(Mutex), map.key_size);
...
// elm in indirections points to entry start ([Mutex | key | value]);
// key comparison uses elm + sizeof(Mutex).
static void _hashmap_find(ConcurrentHashMap& map, size_t hash, void* key, FindResult& result)
{
	result.found = false;
		...
		if (match != 0)
		{
			...
			while (i_low <= i_high)
			{
				if ((match & (1 << i_low)) > 0)
				{
					...
					if (map.cmpfunc((uint8_t*)elm + sizeof(Mutex), key, map.key_size) == 0)
					{
						result.found = true;
...
int create_concurrent_hashmap(ConcurrentHashMap& map, size_t key_size, size_t value_size, size_t(*hfunc)(void*, size_t), int(*cmpfunc)(void*, void*, size_t))
{
	return create_concurrent_hashmap(map, key_size, value_size, 0, 0, hfunc, cmpfunc);
}

int create_concurrent_hashmap(ConcurrentHashMap& map, size_t key_size, size_t value_size, size_t data_vec_size, size_t capacity, size_t(*hfunc)(void*, size_t), int(*cmpfunc)(void*, void*, size_t))
{
	map.meta = nullptr;
	...
	{
		group_count += 1;
		capacity = group_count * group_size;
	}
	...
	size_t entry_size = sizeof(Mutex) + key_size + value_size;
	...
	create_mutex(map.global_mutex);
	create_mutex(map.read_mutex);
	...
	while (i < data_count)
	{
		...
		if (error != E_OK)
		{
			...
			destroy_mutex(map.read_mutex);
			destroy_mutex(map.global_mutex);
...
{
	mutex_lock(map.global_mutex);
	mutex_lock(map.read_mutex);
	...
	mutex_unlock(map.read_mutex);
	mutex_unlock(map.global_mutex);
	destroy_mutex(map.read_mutex);
	destroy_mutex(map.global_mutex);
}
...
{
	ConcurrentHashMapResult result;
	return concurrent_hashmap_insert(map, key, value, result);
}

int concurrent_hashmap_insert(ConcurrentHashMap& map, void* key, void* value, ConcurrentHashMapResult& map_result)
{
	...
	if (result.found)
	{
		...
		map_result.key   = (uint8_t*)ptr + sizeof(Mutex);
		map_result.value = (uint8_t*)ptr + sizeof(Mutex) + map.key_size;
		map_result.mutex = (Mutex*)ptr;
	...
	do
	{
		...
		if (match != 0)
		{
			...
			Vec* vec = (map.data + data_index);
			// entry layout: [Mutex | key | value]
			void* elm = (uint8_t*)vec->data + atdata_index * map.entry_size;

			memcpy((uint8_t*)elm + sizeof(Mutex), key, map.key_size);
			memcpy((uint8_t*)elm + sizeof(Mutex) + map.key_size, value, map.value_size);
			...
			map_result.present = true;
			map_result.key   = (uint8_t*)elm + sizeof(Mutex);
			map_result.value = (uint8_t*)elm + sizeof(Mutex) + map.key_size;
			map_result.mutex = (Mutex*)elm;
...
void concurrent_hashmap_get(ConcurrentHashMap& map, void* key, ConcurrentHashMapResult& map_result)
{
	map_result.present = false;
...
int concurrent_hashmap_remove(ConcurrentHashMap& map, void* key)
{
	...
	if (result.found)
	{
		...
		{
			size_t* ptr = (size_t*)vec_get(*occ_indexes, i, error);
			if (error != E_OK)
			{
				mutex_unlock(map.read_mutex);
				mutex_unlock(map.global_mutex);
...
int concurrent_hashmap_iter_next(ConcurrentHashMapIter& iter, ConcurrentHashMapResult& result)
{
	...
	ptr = vec->data + atdata_index * iter.map->entry_size;

	result.present = true;
	result.key   = (uint8_t*)ptr + sizeof(Mutex);
	result.value = (uint8_t*)ptr + sizeof(Mutex) + iter.map->key_size;
	result.mutex = (Mutex*)ptr;
...
{
	mutex_unlock(iter.map->read_mutex);
}
